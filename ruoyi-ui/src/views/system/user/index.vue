<template>
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="4" :xs="24">
        <div class="head-container">
          <el-input clearable placeholder="请输入部门名称" prefix-icon="el-icon-search" size="small" style="margin-bottom: 20px"
                    v-model="deptName"/>
        </div>
        <div class="head-container">
          <el-tree :data="deptOptions" :expand-on-click-node="false" :filter-node-method="filterNode"
                   :props="defaultProps" @node-click="handleNodeClick" default-expand-all ref="tree"/>
        </div>
      </el-col>
      <!--用户数据-->
      <el-col :span="20" :xs="24">
        <el-form :inline="true" :model="queryParams" label-width="68px" ref="queryForm" v-show="showSearch">
          <el-form-item label="用户名称" prop="userName">
            <el-input @keyup.enter.native="handleQuery" clearable placeholder="请输入用户名称" size="small" style="width: 240px"
                      v-model="queryParams.userName"/>
          </el-form-item>
          <el-form-item label="手机号码" prop="phonenumber">
            <el-input @keyup.enter.native="handleQuery" clearable placeholder="请输入手机号码" size="small"
                      style="width: 240px" v-model="queryParams.phonenumber"/>
          </el-form-item>
          <el-form-item label="状态" prop="status">
            <el-select clearable placeholder="用户状态" size="small" style="width: 240px" v-model="queryParams.status">
              <el-option :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue"
                         v-for="dict in statusOptions"/>
            </el-select>
          </el-form-item>
          <el-form-item label="创建时间">
            <el-date-picker end-placeholder="结束日期" range-separator="-" size="small" start-placeholder="开始日期"
                            style="width: 240px" type="daterange" v-model="dateRange"
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button @click="handleAdd" icon="el-icon-plus" size="mini" type="primary"
                       v-hasPermi="['system:user:add']">新增
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button :disabled="single" @click="handleUpdate" icon="el-icon-edit" size="mini" type="success"
                       v-hasPermi="['system:user:edit']">修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button :disabled="multiple" @click="handleDelete" icon="el-icon-delete" size="mini" type="danger"
                       v-hasPermi="['system:user:remove']">删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button @click="handleImport" icon="el-icon-upload2" size="mini" type="info"
                       v-hasPermi="['system:user:import']">导入
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button @click="handleExport" icon="el-icon-download" size="mini" type="warning"
        ...
        <el-table :data="userList" @selection-change="handleSelectionChange" v-loading="loading">
          ...
          <el-table-column align="center" label="状态">
            <template slot-scope="scope">
              <el-switch @change="handleStatusChange(scope.row)" active-value="0" inactive-value="1"
          ...
          <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="180">
            <template slot-scope="scope">
              ...
              <el-button @click="handleResetPwd(scope.row)" icon="el-icon-key" size="mini" type="text"
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="归属部门" prop="deptId">
              <treeselect :disable-branch-nodes="true" :options="deptOptions" :show-count="true" placeholder="请选择归属部门"
                          v-model="form.deptId"/>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="12">
            <el-form-item label="用户性别">
              <el-select placeholder="请选择" v-model="form.sex">
                <el-option :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue"
                           v-for="dict in sexOptions"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="状态">
              <el-radio-group v-model="form.status">
                <el-radio :key="dict.dictValue" :label="dict.dictValue" v-for="dict in statusOptions">
                  {{dict.dictLabel}}
                </el-radio>
              </el-radio-group>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="岗位">
              <el-select multiple placeholder="请选择" v-model="form.postIds">
                <el-option :disabled="item.status == 1" :key="item.postId" :label="item.postName" :value="item.postId"
          ...
          <el-col :span="12">
            <el-form-item label="角色">
              <el-select multiple placeholder="请选择" v-model="form.roleIds">
                <el-option :disabled="item.status == 1" :key="item.roleId" :label="item.roleName" :value="item.roleId"
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" append-to-body width="400px">
      <el-upload :action="upload.url + '?updateSupport=' + upload.updateSupport" :auto-upload="false" :disabled="upload.isUploading" :headers="upload.headers"
                 :limit="1" :on-progress="handleFileUploadProgress"
                 :on-success="handleFileSuccess" accept=".xlsx, .xls" drag ref="upload">
